...
real &matrix::Matrix::operator()(int row, int col)
{
    if (row >= this->rows_) 
    {
...
real matrix::Matrix::operator()(int row, int col) const
{
    if (row >= this->rows_) 
    {
        std::cerr << "Matrix: row number out of bounds" << std::endl;        
...
/// @return Поток вывода
std::ostream& matrix::operator<<(std::ostream &os, const Matrix &self)
{
...
/// @return Поток ввода
std::istream& matrix::operator>>(std::istream &is, Matrix &self)
{
...
Matrix &matrix::Matrix::operator+=(const Matrix &A)
{
    *this = *this + A;
    return *this;
}
